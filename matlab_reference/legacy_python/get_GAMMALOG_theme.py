...
def get_GAMMALOG_theme(UTM_X, UTM_Y, terrain, complexity, reg, include_peatlands, Npreq, NPL, Nlay):
    # Load data based on region
    region_files = {
        'Jylland': 'Jylland_gammalog_til_usikkerhed.mat',
        'Fyn': 'fyn_gammalog_til_usikkerhed.mat',
        'Sjælland': 'Sjaelland_gammalog_til_usikkerhed.mat'
    }
    ...
    coords = MEP['InfoTable'][['xutm32euref89', 'xutm32euref89']].values
    ...
    # Certainty function for GAMMALOG
    ...
    #print(f'printing rangemap {rangemap}')
    #print(f'printing local_depth {local_depth}')
    var0map = (0.5 * 3 + local_depth * 0)**2
    var0map[local_depth < 3] = 100000
    #print(f'printing var0map {var0map}')
    kernelmap = np.full_like(local_depth, 150)
    #print(f'printing kernelmap {kernelmap}')
    certmap = cert_fun(local_dist, rangemap, kernelmap, 1.0 / var0map)
    #print(f'printing certmap {certmap}')
    Grid = 1.0 / certmap

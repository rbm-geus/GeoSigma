function [theme,Data_snap,Data_other] = get_PL_themes(UTM_X,UTM_Y,terrain,complexity,NL,reg,S)
...
VT0 = V.Jup_bor_data_q(:,["XUTM","YUTM","MAX_DEPTH"]);
...
for i = 1:NL
    ...
    if sum(filter)>0
        ...
        curPeatlandDepths = curPeatlandDepth(ylist(filter),xlist(filter));

        %If a well is deeper than the layer boundary then I use it (with a
        %margin of error of about 1m)
        Tolerance = 1;
        depth_logical = (curDepths+Tolerance)>=curPeatlandDepths;
        ...
        for j = 1:nx
            ...
            if nwells > 0
                ...
                for a = near_well_indices
                    ...
                    curx = curwell(1);
                    cury = curwell(2);
                    curd = max(curwell(4)-curwell(3),0);
                    curc = curwell(5);

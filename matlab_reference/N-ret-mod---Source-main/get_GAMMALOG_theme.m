function [Grid,s_cords] = get_GAMMALOG_theme(UTM_X,UTM_Y,terrain,complexity,reg,include_peatlands,Npreq,NPL,Nlay)

%Load File based on region
switch reg
    case 'Jylland'
        filename = 'jylland_gammalog_til_usikkerhed.mat';
    case 'Fyn'
        filename = 'fyn_gammalog_til_usikkerhed.mat';
    case 'Sjælland'
        filename = 'Sjaelland_gammalog_til_usikkerhed.mat';
end
...
[~,id,~] = unique(table2array(MEP.InfoTable(:,["xutm32euref89","xutm32euref89"])),'rows');
...
%    cert_fun = @(dist,range,width,sill) (dist<=width).*sill+(dist>width).*sill.*(exp(-(3*power((dist-width),2))./(power(range,2))));
    ...
    local_model_thick = NaN([size(terrain),maxl]);   %indsæt
        ...
        parfor i = 1:ny
            ...
            for j = 1:nx
                ...
                if npoints > 0
                    ...
                    local_model_thick(i,j,s) = min(local_thicks(filt_mindist));   %ret
...
%Calculate var0 map
var0map = (0.5*3 +local_depth*0).^2;      %rettet
var0map(local_depth<3)=100000;            %indsat

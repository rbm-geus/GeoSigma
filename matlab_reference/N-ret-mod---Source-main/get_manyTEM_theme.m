function [Grid,s_cords] = get_manyTEM_theme(UTM_X,UTM_Y,terrain,complexity,reg,include_peatlands,Npreq,NPL,Nlay)

%Load File based on region
switch reg
    case 'Jylland'
        filename = 'jylland_tem_mangelag_til_usikkerhed.mat';
    case 'Fyn'
        filename = 'fyn_tem_til_usikkerhed.mat';
    case 'Sjælland'
        filename = 'Sjaelland_tem_mangelag_til_usikkerhed.mat';
end
...
%     cert_fun = @(dist,range,width,sill,d0) (dist<=width).*sill+(dist>width).*sill.*(exp(-(3*power((dist-width),2))./(power(range,2))));
    ...
    doimean(isnan(doimean))=150;              % Jylland ingen DOI for data i dette tema. DOI-mean ca. fra fewTEM i Jylland

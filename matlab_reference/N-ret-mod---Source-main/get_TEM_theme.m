function [Grid,s_cords] = get_TEM_theme(UTM_X,UTM_Y,terrain,complexity,reg,include_peatlands,Npreq,NPL,Nlay,cert_fun_choice)

%Load File based on region
switch reg
    case 'Jylland'
        filename = 'jylland_tem_til_usikkerhed.mat';
    case 'Fyn'
        filename = 'fyn_tem_til_usikkerhed.mat';
    case 'Fyn-MST'
        filename = 'fyn_tem_til_usikkerhed_MST.mat';
    case 'Sjælland'
        filename = 'sjælland_tem_til_usikkerhed.mat';
end
...
if NP > 0
    ...
    for s = 1:(Nlay-NPL)
        disp(['Calculating TEM theme for layer ' num2str(s) ' (that is not peatland) using ' cert_fun_choice])
